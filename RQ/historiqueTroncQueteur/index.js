      ...
      .then(queteurPromise => {
        if (queteurPromise.exists)
        {
          ...
          if( historiqueTQLastUpdate != null &&
              moment().diff(historiqueTQLastUpdate, 'seconds') <= 5*60 &&
              Array.isArray(historiqueTQ) &&
             (historiqueTQ.length > 0 || moment().diff(historiqueTQLastUpdate, 'seconds') <= 60)
          )
            ...
            mysqlPool.query(
              ...
              (err, results) => {
                ...
                else
                {
                  ...
                  queteurRef.update({'historiqueTQ':JSON.parse(JSON.stringify(results)), 'historiqueTQLastUpdate': JSON.parse(JSON.stringify(new Date()))}).then(()=>{
                    resolve(results);
                  }).catch(updateError=>{
